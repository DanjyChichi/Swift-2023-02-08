...
//비동기 처리 : 예)통신 - HTTP통신에서 요청과 응답의 시점이 다름. 요청하는 코드와 응답받고 처리할 코드를 함께 기술해 놓으면 매우 편리하다.
...
//클로저 선언 = 코드블럭를 변수형태로 담은 것 : 함수형 변수
var sum : (Int, Int) -> Int = {
    (a:Int, b:Int) -> Int in
        return a + b
}
//클로저 실행
result = sum( 10, 20 );
//클로저를 만드는 이유: 코드블럭을 변수로 만들어서 매개변수로
//                전달하기 쉽다!
